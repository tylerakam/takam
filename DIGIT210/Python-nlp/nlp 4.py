...
newStopWords = ["1", "v1", "well", "2", "3", "4", "dont", "msgs", "im", "c"]
...
pFloydPath = os.path.join(workingDir, 'TWDep')
print(pFloydPath)
...
allDocs = []
for file in os.listdir(pFloydPath):
    if file.endswith(".txt"):
        filepath = f"{pFloydPath}/{file}"
        # print(filepath)
...
sorted_topics = sorted(topics[50], key=lambda x: x[1], reverse=True)
